...
function TwinTrap_Exp(zp, range, range_N, hemis_r, xslice, yslice, zslice, sp_number, saveName, movefilename, titleName)
    %% bottle trap用の超音波振動子配置を行う
    %     sp_number = 100;
%     hemis_r = 50;
    rangez_up = 2*8.5; rangez_bottom = -2*8.5;
%     range = 30; 
%     range_N = 100;
%     zp = 0;
%     RowsCombNum = [1 2 3 4];
...
%     for theta = linspace(pi,pi/2,howmany_row)
    ...
    %         end
            sum_sp = sum_sp + howmany_div4rr*4;
            for j = 0:3
    ...
    %               end
            ...
            lastRow_spNum = sp_number - sum_sp;
            lastRow_divTheta = 2*pi/lastRow_spNum;
            sum_sp = sum_sp + lastRow_spNum;
            for phai = linspace(0,2*pi-lastRow_divTheta,lastRow_spNum)
                  x = hemis_r*sin(theta)*cos(phai);
                  y = hemis_r*sin(theta)*sin(phai);
                  z = -hemis_r*cos(theta)+z_position;
                  sp_list(i) = UltrasonicSpeaker3D_3(x, y, z,f_x,f_y,f_z,f); 
    ...
    view(90,0)
    name =strcat('TwinDesign_R_',num2str(hemis_r)); 
...
%     movefile(strcat('kakuninup_z',num2str(nameIndex),'.png'), movefilename);
    ...
    name =strcat('TwinDesign_R_',num2str(hemis_r),'.fig'); 
    saveas(gcf,name);
    movefile(name, movefilename);
    
    csvwrite_spPositionColor_paraview_2; %csvfileに振動子の位置を書き出す
...
%             end               
        ...
        colormap(hsv);
        saveSimuFig_2(saveName,'_P_angle',movefilename);
%         
...
%         P = P_avarage.*P_angle_negaPosi; 
        ...
        %saveSimuFig_2(saveName,strcat('_',num2str(t_idx)),movefilename);
%         saveSimuFig_2(saveName,'_P_avarage',movefilename);
        saveSimuFig(saveName,'_P_avarage',movefilename,P_avarage);
