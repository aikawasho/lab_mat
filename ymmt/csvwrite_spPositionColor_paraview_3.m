...
%         csv_w(csv_index,:) = { strcat('[', num2str(obj.x0)), obj.y0, obj.z0, obj.v_x, obj.v_y, obj.v_z, 'red]'}; % matlabでのxy軸がparaviewではyx軸となるのでi,jを入れ替えてある
        csv_w(csv_index,:) = { obj.y0, obj.x0, obj.z0, obj.v_y, obj.v_x, obj.v_z, 0}; % matlabでのxy軸がparaviewではyx軸となるのでi,jを入れ替えてある. また、colorカラムの0はredを意味する。
        csv_index = csv_index + 1;  
%     elseif obj.qui_color == 'blue'
    elseif strcmp(obj.qui_color, 'blue')
        csv_w(csv_index,:) = { obj.y0, obj.x0, obj.z0, obj.v_y, obj.v_x, obj.v_z, 1}; % matlabでのxy軸がparaviewではyx軸となるのでi,jを入れ替えてある. また、colorカラムの1はredを意味する。
        csv_index = csv_index + 1;    
    end
...
export(csv_w_dataset,'File','spPosition.csv','Delimiter',',')
