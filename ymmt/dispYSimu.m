...
function dispYSimu(titleName,X,Y,Z,P)
     % simulationの表示
    figure
    x0=10;
    y0=10;
...
%     title(strcat(num2str(rad2deg(phase)),' degree'));
    ...
    ax2 = gca;
    axes(ax2);
    yslice_plot = slice(X,Y,Z,P,[],yslice,[]);
    volumeData = get(yslice_plot,'CData');
...
%     pbaspect([1 1 1]) % graph縦横比
    ...
    set(ax2, 'FontSize', 22);
    set(yslice_plot,'LineStyle','none')  
    cbar = colorbar;
...
%     set(ax2, 'CLim', [minVolumeData maxVolumeData]);
    try
        ...
        warning('CLimがセットできませんでした。volumeDataにNanが含まれているのかもしれません。')
    end
