...
fi.InnerPosition =[0,0,800,700];
hold on
bi = 0;
y_tic = 0:0.20:0.20*(length(tp_z)-1);

for num = 1:length(tp_z)    
    name = sprintf('%.1f mm ', tp_z(num));
    plot(z,st_Z(:,num)+0.20*(num-1),'LineWidth',2,'DisplayName',name);
    ax = gca;
    ax.FontSize = 15;
    %ylim([-0.3 0.3])
    xlabel('z (mm)');
...
legend show
if save_graph == 1
    file_name = strcat(Fname,'z_z.png');
    saveas(gcf,file_name)
end

fi = figure(4);
fi.InnerPosition =[900,0,800,700];
hold on
bi = 0;
y_tic = 0:0.15:0.15*(length(tp_z)-1);
for num = 1:length(tp_z)    
    name = sprintf('%.1f mm ', tp_z(num));
    plot(z,st_Zy(:,num)+0.15*(num-1),'LineWidth',2,'DisplayName',name);
    ax = gca;
    ax.FontSize = 15;
    %ylim([0 0.3])
    xlabel('z (mm)');
...
legend show
if save_graph == 1
    file_name = strcat(Fname,'z_y.png');
    saveas(gcf,file_name)
end